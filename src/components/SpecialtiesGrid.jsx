...
import { X, Search, ChevronRight, MapPin, Link as LinkIcon, Stethoscope, User, Phone } from 'lucide-react';
import {
...
const specialtiesData = [
    { id: "anestesiologia", name: "Anestesiologia", description: "Segurança cirúrgica e bem-estar.", icon: SyringeOutline, doctors: [] },
    { id: "cardiologia", name: "Cardiologia", description: "Saúde do coração.", icon: HeartOutline, doctors: [] },
    { id: "cirurgia-geral", name: "Cirurgia Geral", description: "Procedimentos cirúrgicos diversos.", icon: StethoscopeOutline, doctors: [] },
    { id: "cirurgia-vascular", name: "Cirurgia Vascular", description: "Saúde das veias e artérias.", icon: HeartOrganOutline, doctors: [] },
    { id: "cirurgia-plastica", name: "Cirurgia Plástica", description: "Procedimentos estéticos e reparadores.", icon: BodyOutline, doctors: [] },
    { id: "gastro", name: "Gastroenterologia e Aparelho Digestivo", description: "Saúde do aparelho digestivo.", icon: StomachOutline, doctors: [] },
    { id: "ginecologia", name: "Ginecologia", description: "Saúde da mulher.", icon: FemaleReproductiveSystemOutline, doctors: [] },
    { id: "neurologia", name: "Neurologia", description: "Saúde do sistema nervoso.", icon: HeadOutline, doctors: [] },
    { id: "oftalmologia", name: "Oftalmologia", description: "Clínica ocular e cirurgias.", icon: EyeOutline, doctors: [] },
    { id: "ortopedia", name: "Ortopedia", description: "Atendimento dos ossos e articulações.", icon: JointsOutline, doctors: [] },
    { id: "otorrino", name: "Otorrinolaringologia", description: "Ouvido, nariz e garganta.", icon: EarOutline, doctors: [] },
    { id: "proctologia", name: "Proctologia", description: "Saúde do intestino e reto.", icon: IntestineOutline, doctors: [] },
    { id: "urologia", name: "Urologia", description: "Trato urinário e reprodutor.", icon: KidneysOutline, doctors: [] },
];
...
export default function SpecialtiesGrid() {
    ...
    useEffect(() => {
        ...
        loadDoctors();
        console.log("Médicos carregados no Site:", localStorage.getItem('hosp_doctors_db'));
    ...
    const specialtiesWithDoctors = useMemo(() => {
        return specialtiesData.map(spec => ({
            ...
            doctors: allDoctors.filter(doc => {
                ...
                return docSpec === specName || docSpec.includes(specName) || specName.includes(docSpec);
            }).map(doc => ({
                id: doc.id,
                name: doc.name,
                crm: doc.crm,
    ...
    const filteredSpecialties = useMemo(() => {
        return specialtiesWithDoctors.filter(spec =>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filteredSpecialties.length > 0 ? (
                        filteredSpecialties.map((spec) => (
                            ...
                            >
                                ...
                                <div className="flex-grow">
                                    <h3 className="text-lg font-bold text-medical-dark group-hover:text-medical-blue transition-colors">{spec.name}</h3>
                                    <p className="text-sm text-slate-500 line-clamp-1">{spec.description}</p>
                                </div>
                        ...
                        <div className="col-span-full py-12 text-center text-slate-500 bg-white rounded-3xl border border-dashed border-gray-200">
                            ...
                            <p className="text-lg font-medium">Nenhuma especialidade encontrada para "{searchQuery}"</p>
                        </div>
                    )}
                </div>
                    ...
                    {selectedSpecialty && (
                        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center sm:p-4">
                            <motion.div
                                ...
                                onClick={() => { setSelectedSpecialty(null); setSelectedDoctor(null); }}
                            />
                                    ...
                                    <div className="pr-4">
                                        <div className="flex items-center gap-3 mb-2">
                                            <div className="text-medical-blue bg-medical-light/50 p-2.5 rounded-xl border border-medical-blue/10 w-12 h-12 flex items-center justify-center">
                                                {selectedSpecialty.icon && React.createElement(selectedSpecialty.icon, { className: "w-7 h-7 fill-current" })}
                                            </div>
                                            <h3 className="text-2xl md:text-3xl font-bold text-medical-dark font-heading leading-tight">{selectedSpecialty.name}</h3>
                                        </div>
                                        {selectedDoctor ? (
                                            <button onClick={() => setSelectedDoctor(null)} className="text-sm font-bold text-medical-blue hover:text-medical-accent flex items-center gap-1 group">
                                                <ChevronRight size={16} className="rotate-180 group-hover:-translate-x-1 transition-transform" />
                                                Voltar para os médicos
                                            </button>
                                        ) : (
                                            <p className="text-sm text-slate-500">Selecione o profissional para agendar.</p>
                                        )}
                                    </div>
                                    <button
                                        onClick={() => { setSelectedSpecialty(null); setSelectedDoctor(null); }}
                                        ...
                                        <div className="grid grid-cols-1 gap-3">
                                            {selectedSpecialty.doctors.length > 0 ? (
                                                selectedSpecialty.doctors.map((doc, i) => (
                                                    ...
                                                    >
                                                        <img src={doc.img} alt={doc.name} className="w-16 h-16 md:w-20 md:h-20 rounded-full object-cover border-2 border-medical-light/50 shadow-sm" />
                                                        <div className="flex-grow">
                                                            <h4 className="font-bold text-lg text-medical-dark group-hover:text-medical-blue transition-colors leading-tight">{doc.name}</h4>
                                                            <p className="text-sm text-slate-500 mt-0.5">{doc.crm}</p>
                                                        </div>
                                                ...
                                                <div className="text-center py-16 px-4 bg-white rounded-2xl border border-dashed border-gray-200">
                                                    ...
                                                    <h4 className="font-bold text-lg text-medical-dark mb-2">Corpo Clínico em Atualização</h4>
                                                    <p className="text-slate-500 text-sm max-w-sm mx-auto">Em breve disponibilizaremos a lista completa de profissionais desta especialidade.</p>
                                                </div>
                                            )}
                                        </div>
                                    ) : (
                                            ...
                                            <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm w-full">
                                                <div className="flex flex-col sm:flex-row gap-6 items-center sm:items-start text-center sm:text-left">
                                                    ...
                                                    <div className="flex-grow w-full">
                                                        <h4 className="text-2xl font-bold font-heading text-medical-dark mb-1">{selectedDoctor.name}</h4>
                                                        <p className="text-medical-blue font-bold text-sm bg-medical-light/30 inline-block px-3 py-1 rounded-full mb-4">{selectedDoctor.crm}</p>

                                                        <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-4">
                                                            {selectedDoctor.instagram && (
                                                                <a 
                                                                    href={selectedDoctor.instagram.startsWith('http') ? selectedDoctor.instagram : `https://instagram.com/${selectedDoctor.instagram.replace('@', '')}`} 
                                                                    target="_blank" 
                                                                    rel="noreferrer" 
                                                                ...
                                                                >
                                                                    ...
                                                                    {selectedDoctor.instagram.startsWith('http') ? 'Instagram' : selectedDoctor.instagram}
                                                                </a>
                                                            )}
                                                            {selectedDoctor.site && (
                                                                <a 
                                                                    href={selectedDoctor.site.startsWith('http') ? selectedDoctor.site : `https://${selectedDoctor.site}`} 
                                                                    target="_blank" 
                                                                    rel="noreferrer" 
                                                ...
                                                <div className="space-y-5 text-left">
                                                    <div>
                                                        <h5 className="font-bold text-medical-dark mb-2 text-sm uppercase tracking-wider flex items-center gap-2">
                                                            <User size={16} className="text-medical-blue" /> Sobre o Profissional
                                                        </h5>
                                                        <p className="text-slate-600 text-sm md:text-base leading-relaxed">
                                                            {selectedDoctor.bio || 'Sem informações adicionais.'}
                                                        </p>
                                                    </div>
                                                    ...
                                                    {selectedDoctor.address && (
                                                        <div>
                                                            <h5 className="font-bold text-medical-dark mb-2 text-sm uppercase tracking-wider flex items-center gap-2">
                                                                <MapPin size={16} className="text-medical-blue" /> Endereço de Atendimento
                                                            </h5>
                                                            <p className="text-slate-600 text-sm md:text-base leading-relaxed">
                                                                {selectedDoctor.address}
                                                            </p>
                                                    ...
                                                    {selectedDoctor.phone && (
                                                        <div>
                                                            <h5 className="font-bold text-medical-dark mb-2 text-sm uppercase tracking-wider flex items-center gap-2">
                                                                <Phone size={16} className="text-medical-blue" /> Telefone para Contato
                                                            </h5>
                                                            <p className="text-slate-600 text-sm md:text-base leading-relaxed">
                                                                {selectedDoctor.phone}
                                                            </p>
                                                ...
                                                <div className="mt-8 pt-6 border-t border-gray-100">
                                                    <button
                                                        onClick={() => handleWhatsApp(selectedDoctor.phone, selectedDoctor.name)}
