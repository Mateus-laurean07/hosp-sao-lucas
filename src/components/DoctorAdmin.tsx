...
export default function DoctorAdmin() {
  ...
  const [selectedDoctorView, setSelectedDoctorView] = useState<Doctor | null>(
    null,
  );
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > 1.5 * 1024 * 1024) {
        alert(
          "A imagem é muito grande. Por favor, escolha uma imagem com menos de 1.5MB para garantir o salvamento.",
        );
        return;
      }
  ...
  const filteredDoctors = useMemo(() => {
    ...
    return doctors.filter(
      (d) => d.specialty.toLowerCase() === selectedSpecialty.name.toLowerCase(),
    );
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col items-center p-4 py-12 font-sans relative">
      <div
        className={`w-full ${!selectedSpecialty ? "max-w-6xl" : "max-w-3xl"} z-10 transition-all duration-500`}
      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {specialtiesData.map((spec) => (
                <div
                  ...
                  className="bg-white rounded-2xl p-5 flex items-center gap-4 hover:shadow-xl border border-gray-100 cursor-pointer transition-all group active:scale-95 shadow-sm"
                >
                  <GlowingIcon icon={spec.icon} />
                  <div className="flex-grow min-w-0">
                    <h3 className="text-[17px] font-bold text-[#073a78] group-hover:text-[#0b5bbb] transition-colors truncate">
                      {spec.name}
                    </h3>
                  ...
                  <ChevronRight
                    size={20}
                    className="text-slate-200 group-hover:text-[#0b5bbb] group-hover:translate-x-1 transition-all"
                  />
                </div>
              ))}
            </div>
            ...
            <div className="p-8 border-b border-gray-100 flex items-start justify-between bg-white sticky top-0 z-20">
              <div className="pr-4 min-w-0">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="text-2xl md:text-3xl font-bold text-[#073a78] font-heading leading-tight truncate">
                ...
                {selectedDoctorView ? (
                  <button
                    ...
                    className="text-sm font-bold text-[#0b5bbb] hover:text-[#073a78] flex items-center gap-1 group transition-all"
                  >
                    <ChevronLeft
                      size={16}
                  ...
                  <p className="text-sm text-slate-400 font-semibold flex items-center gap-4">
                    ...
                    <button
                      ...
                      className="text-xs font-bold text-[#0b5bbb] hover:underline underline-offset-2 tracking-tight"
                    >
                      Especialidades
                    </button>
                  </p>
              ...
              <div className="flex items-center gap-2 shrink-0">
                {!selectedDoctorView && (
                  <button
                    onClick={() => openForm(selectedSpecialty.name)}
                    className="flex items-center gap-1.5 px-4 py-2.5 bg-[#0b5bbb] text-white rounded-xl font-bold hover:bg-[#073a78] transition-all shadow-md shadow-blue-500/20 active:scale-95 text-sm"
                  >
                  ...
                  {filteredDoctors.length > 0 ? (
                    filteredDoctors.map((doc) => (
                      <div
                        ...
                        className="flex items-center gap-4 bg-white border border-gray-100 p-4 font-medium rounded-2xl hover:border-blue-200 hover:shadow-lg transition-all cursor-pointer group relative shadow-sm"
                      >
                        ...
                        <div className="flex-grow min-w-0 pr-16">
                          <h4 className="font-bold text-lg text-[#073a78] group-hover:text-[#0b5bbb] transition-colors leading-tight truncate">
                            {doc.name}
                          </h4>
                        ...
                        <div className="absolute right-4 flex gap-1">
                          <button
                            onClick={(e) =>
                              openForm(selectedSpecialty.name, doc, e)
                            }
                            className="p-2.5 bg-blue-50 text-[#0b5bbb] hover:bg-[#0b5bbb] hover:text-white rounded-xl transition-all active:scale-90"
                            title="Editar"
                          >
                            <Pencil size={18} />
                          </button>
                    ...
                    <div className="flex flex-col items-center justify-center py-24 text-center opacity-70">
                      <div className="w-20 h-20 bg-white rounded-[2rem] flex items-center justify-center text-slate-200 mb-5 shadow-sm border border-slate-50">
                        <User size={40} />
                      </div>
                      <h4 className="text-[#073a78] font-bold text-xl mb-1">
                        Nenhum profissional listado
                      </h4>
                      <p className="text-slate-400 text-sm font-medium">
                        Adicione médicos para esta especialidade.
                      </p>
                    </div>
                  )}
                </div>
                ...
                >
                  <div className="bg-white p-8 md:p-12 rounded-[2.5rem] border border-gray-100 shadow-sm w-full relative">
                    <div className="absolute top-8 right-8 flex gap-2">
                      <button
                        onClick={() =>
                          openForm(selectedSpecialty.name, selectedDoctorView)
                        }
                        className="p-3 bg-blue-50 text-[#0b5bbb] rounded-2xl hover:bg-blue-100 transition-colors"
                        title="Editar"
                      >
                        <Pencil size={20} />
                      </button>
                      <button
                        onClick={() => setDoctorToDelete(selectedDoctorView.id)}
                    ...
                    <div className="flex flex-col sm:flex-row gap-10 items-center sm:items-start text-center sm:text-left mb-10">
                      <div className="w-40 h-40 md:w-52 md:h-52 shrink-0 rounded-[3rem] overflow-hidden shadow-2xl border-4 border-white bg-slate-50">
                        {selectedDoctorView.photoUrl ? (
                          <img
                            src={selectedDoctorView.photoUrl}
                            alt={selectedDoctorView.name}
                            className="w-full h-full object-cover"
                          />
                        ) : (
                      ...
                      <div className="flex-grow pt-4">
                        <h4 className="text-4xl font-extrabold font-heading text-[#073a78] mb-3 tracking-tight">
                          {selectedDoctorView.name}
                        </h4>
                        <div className="inline-block px-5 py-2 bg-[#f0f7ff] text-[#0b5bbb] rounded-full font-black text-sm border border-blue-50 shadow-sm mb-8 tracking-widest">
                          {selectedDoctorView.crm}
                        </div>
                        ...
                        <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4">
                          {selectedDoctorView.instagramUrl && (
                            <a
                              href={
                                selectedDoctorView.instagramUrl.startsWith(
                                  "http",
                                )
                                  ? selectedDoctorView.instagramUrl
                                  : `https://instagram.com/${selectedDoctorView.instagramUrl.replace("@", "")}`
                              }
                              target="_blank"
                          ...
                          {selectedDoctorView.websiteUrl && (
                            <a
                              href={
                                selectedDoctorView.websiteUrl.startsWith("http")
                                  ? selectedDoctorView.websiteUrl
                                  : `https://${selectedDoctorView.websiteUrl}`
                              }
                              target="_blank"
                              rel="noopener noreferrer"
                    ...
                    <div className="grid gap-10 border-t border-gray-100 pt-12">
                      <div>
                        <h5 className="font-black text-[#073a78] mb-4 text-[11px] uppercase tracking-[0.2em] flex items-center gap-2">
                          <User size={16} className="text-[#0b5bbb]" /> SOBRE O
                          PROFISSIONAL
                        </h5>
                        <p className="text-slate-600 text-lg leading-relaxed font-medium">
                          {selectedDoctorView.about ||
                            "A equipe do Hospital São Lucas ainda não informou os dados biográficos deste profissional."}
                        </p>
                      </div>
                      ...
                      <div className="grid sm:grid-cols-2 gap-12">
                        <div>
                          <h5 className="font-black text-[#073a78] mb-4 text-[11px] uppercase tracking-[0.2em] flex items-center gap-2">
                            <MapPin size={16} className="text-[#0b5bbb]" />{" "}
                            ENDEREÇO CLÍNICA
                          </h5>
                          <p className="text-slate-700 text-[15px] font-bold leading-relaxed">
                            {selectedDoctorView.address ||
                              "Consultar na recepção do Hospital."}
                          </p>
                        </div>
                        <div>
                          <h5 className="font-black text-[#073a78] mb-4 text-[11px] uppercase tracking-[0.2em] flex items-center gap-2">
                            <Phone size={16} className="text-[#0b5bbb]" />{" "}
                            CONTATO AGENDAMENTO
                          </h5>
          ...
          {isFormOpen && (
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="absolute inset-0 bg-[#073a78]/70 backdrop-blur-md"
              ...
              <motion.div
                ...
                className="bg-white w-full max-w-xl rounded-[3rem] p-10 md:p-12 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] relative z-10 border-t-[10px] border-[#0b5bbb] text-left max-h-[92vh] overflow-y-auto custom-scrollbar"
              >
                <button
                  onClick={() => setIsFormOpen(false)}
                  className="absolute top-8 right-8 p-2.5 hover:bg-slate-50 rounded-full text-slate-400 transition-all active:rotate-90"
                >
                  <X size={26} />
                </button>

                <div className="mb-10">
                  <h3 className="text-3xl font-black text-[#073a78] leading-tight font-heading tracking-tight">
                    {formData.id ? "Editar Profissional" : "Novo Cadastro"}
                  </h3>
                  <div className="inline-block mt-2 px-3 py-1 bg-blue-50 text-[#0b5bbb] text-[11px] font-black uppercase tracking-widest rounded-lg border border-blue-100">
                    {formData.specialty}
                  </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-7">
                  ...
                  <div className="p-8 bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center text-center transition-all hover:border-[#0b5bbb]/30 group">
                    <div className="relative mb-5">
                      <div className="w-28 h-28 md:w-32 md:h-32 rounded-[2.25rem] overflow-hidden bg-white shadow-xl border-4 border-white relative group-hover:scale-105 transition-transform">
                        ...
                        <button
                          ...
                          className="absolute inset-0 bg-[#073a78]/50 text-white opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center transition-all"
                        >
                    ...
                    <div className="w-full">
                      <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider mb-2 block">
                        Link da Imagem ou Caminho Local
                      </label>
                      <input
                        name="photoUrl"
                        value={
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-1.5">
                      <label className="text-[11px] font-black uppercase text-slate-700 ml-1 tracking-tight">
                        Nome Completo
                      </label>
                      <input
                        required
                        ...
                        value={formData.name}
                        onChange={handleInputChange}
                        className="w-full p-4.5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-[#0b5bbb] focus:bg-white outline-none transition-all font-bold text-slate-700 shadow-sm"
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        onChange={handleInputChange}
                        className="w-full p-4.5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-[#0b5bbb] focus:bg-white outline-none transition-all font-bold text-slate-700 shadow-sm"
                        placeholder="@dramaria_cardiologia"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[11px] font-black uppercase text-slate-700 ml-1 tracking-tight">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      className="w-full p-4.5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-[#0b5bbb] focus:bg-white outline-none transition-all font-bold text-slate-700 shadow-sm"
                      placeholder="https://www.dramaria.com.br"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-black uppercase text-slate-700 ml-1 tracking-tight">
                    ...
                    <textarea
                      ...
                      onChange={handleInputChange}
                      className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-3xl focus:border-[#0b5bbb] focus:bg-white outline-none transition-all h-32 font-semibold text-slate-700 resize-none shadow-sm leading-relaxed"
                      placeholder="Fale um pouco sobre a formação e experiência do médico..."
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-black uppercase text-slate-700 ml-1 tracking-tight">
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      className="w-full p-4.5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-[#0b5bbb] focus:bg-white outline-none transition-all font-bold text-slate-700 shadow-sm"
                      placeholder="Ex: Av. Principal, 123 - Sala 45"
                    />
                  </div>
                  ...
                  <div className="flex gap-4 pt-6 sticky bottom-0 bg-white/90 backdrop-blur-sm mt-4 border-t border-slate-50 py-4">
                    <button
                      type="button"
                      onClick={() => setIsFormOpen(false)}
                      className="flex-1 py-5 bg-slate-100 font-black text-slate-500 rounded-2xl hover:bg-slate-200 transition-all uppercase tracking-widest text-xs"
                    >
                      Cancelar
                    </button>
                    <button
                      type="submit"
                      className="flex-1 py-5 bg-[#0b5bbb] text-white font-black rounded-2xl shadow-xl shadow-blue-500/20 hover:bg-[#073a78] transition-all uppercase tracking-widest text-xs active:scale-[0.98]"
                    >
                      Salvar Profissional
                    </button>
                  </div>
          ...
          {doctorToDelete && (
            <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="absolute inset-0 bg-[#073a78]/80 backdrop-blur-md"
              ...
              <motion.div
                ...
                className="bg-white max-w-sm w-full rounded-[3rem] p-10 text-center relative z-10 shadow-2xl overflow-hidden border-t-8 border-red-500"
              >
                ...
                <h3 className="text-3xl font-black text-[#073a78] mb-4 tracking-tight">
                  Confirmar Exclusão?
                </h3>
                <p className="text-slate-500 font-bold mb-10 leading-relaxed text-sm">
                  O registro deste médico será removido permanentemente de todos
                  os sistemas do Hospital São Lucas.
                </p>
                <div className="flex flex-col gap-4">
                  <button
                    onClick={confirmDelete}
                    className="w-full py-5 bg-red-500 text-white font-black rounded-2xl hover:bg-red-600 shadow-xl shadow-red-500/30 transition-all uppercase tracking-widest text-[11px] active:scale-95"
                  >
                    Sim, Excluir Profissional
                  </button>
                  <button
                    onClick={() => setDoctorToDelete(null)}
                    className="w-full py-5 bg-slate-50 text-slate-400 font-bold rounded-2xl hover:bg-slate-100 transition-all uppercase tracking-widest text-[11px]"
                  >
                    Manter Cadastro
                  </button>
                </div>
      ...
      <style>{`
                ...
                body.admin-modal-open header { display: none !important; }
                body.admin-modal-open .fixed-whatsapp-button { display: none !important; }
            `}</style>
    </div>
  );
}
